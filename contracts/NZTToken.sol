...
library AddressMinHeap {
    ...
    function decode(uint256 _entry) internal pure returns (address _addr, uint256 _value) {
        /* solium-disable-next-line */
        assembly {
            let enter := not(_entry)
            _addr := and(enter, 0xffffffffffffffffffffffffffffffffffffffff)
            _value := shr(160, enter)
        }
    }
...
contract NZTToken is Ownable, GasPump, IBEP20 {
    ...
    function _transferFrom(address _operator, address _from, address _to, uint256 _value, bool _payFee) internal {
        ...
        if (_payFee || !_isWhitelisted(_from, _to)) {
            // Fee is the same for BURN and NZT
            ...
            _receive = _receive.sub(burn.add(nzt));
            ...
            _setBalance(winner, _balanceOf(winner).add(nzt));
            emit Winner(winner, nzt);
            emit Transfer(_from, winner, nzt);
        }

        // Sanity checks
        // no tokens where created
        assert(burn.add(nzt).add(_receive) == _value);

        // Add tokens to receiver
        _setBalance(_to, _balanceOf(_to).add(_receive));
        emit Transfer(_from, _to, _receive);
    }
    ...
    function transfer(address _to, uint256 _value) override  external requestGas(extraGas) returns (bool) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) override  external requestGas(extraGas) returns (bool) {
